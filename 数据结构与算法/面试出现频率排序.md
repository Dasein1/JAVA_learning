...
# 136.==只出现一次的数==
![](images/2022-04-12-20-17-29.png)
## 异或运算
### 思路：
利用异或运算的性质:
...
class Solution {
    public String convert(String s, int numRows) {
        ...
        int cur_row=-1;
        int next_add_index=0;
        int add_num=0;
        char[] s_builder=new char[string_length];
        int cur_index=0;
        while(append_nums!=string_length){
            cur_row++;
            cur_index=cur_row;
            s_builder[append_nums]=s.charAt(cur_index);
            append_nums++;
            next_add_index=0;
            while(append_nums!=string_length){
                if(cur_row==0 || cur_row==numRows-1){
                    ...
                    if(next_add_index==0){
                        add_num=numRows*2-2-2*cur_row;
                        next_add_index=1;
                    }
                    else if(next_add_index==1){
                        add_num=2*cur_row;
                        next_add_index=0;
                    }
                }
                cur_index=cur_index+add_num;
        ...
        return String.valueOf(s_builder);
    }
}
```

# 32.最长有效括号
![](images/2022-05-02-19-56-23.png)
